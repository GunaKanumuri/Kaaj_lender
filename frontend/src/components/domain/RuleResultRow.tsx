// ============================================================
// RuleResultRow — pass/fail row for one PolicyRule evaluation
//
// TABLE OF CONTENTS
//   1.  Props
//   2.  RuleResultRow component
//
// VISUAL STATES
//   Pass       → emerald background, CheckCircle icon
//   Hard fail  → red background, XCircle icon, "hard fail" badge
//   Soft fail  → amber background, AlertCircle icon, "soft" badge
//
// Shows actual vs required values on failure so the broker can
// tell the client exactly what needs to change.
// ============================================================
...
// region ── 2. RuleResultRow component ────────────────────────
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Actual vs required — only shown on failure */}
        {!result.passed && (
          <div className="flex gap-4 mt-1 text-xs">
            <span className="text-zinc-600">
              Got:{' '}
              <span className="font-mono font-medium">{result.actual_value ?? 'N/A'}</span>
            </span>
            <span className="text-zinc-600">
              Required:{' '}
              <span className="font-mono font-medium">{result.required_value ?? 'N/A'}</span>
            </span>
          </div>
        )}
      </div>
